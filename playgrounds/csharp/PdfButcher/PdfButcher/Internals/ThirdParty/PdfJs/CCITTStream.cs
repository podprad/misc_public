...
namespace PdfButcher.Internals.ThirdParty.PdfJs
{
    ...
    internal class CCITTStream : DecodeStream
    {
        ...
        public CCITTStream(int? k, bool? endofLine, bool? endOfBlock, bool? blackIs1, bool? encodedByteAlign, int? columns,
            int? rows, Stream inner, Action<string> errorCallback)
            : base(inner)
        {
                ...
                if (this.nextLine2D)
                {
                    ...
                    while (codingLine[this.codingPos] < columns)
                    {
                        ...
                        switch (code1)
                        {
                            ...
                            default:
                                errorCallback?.Invoke("bad 2d code in ccitt");
                                this._addPixels(columns, 0);
                                this.err = true;
                    ...
                    if (code1 == 1)
                    {
                        ...
                        if (this.encoding >= 0)
                        {
                            for (i = 0; i < 4; ++i)
                            {
                                ...
                                if (code1 != 1)
                                {
                                    errorCallback?.Invoke("bad rtc code in ccitt: " + code1);
                                }
     ...
     * @private
     */
        private void _addPixels(int a1, int blackPixels)
        {
            var codingLine = this.codingLine;
            ...
            if (a1 > codingLine[codingPos])
            {
                if (a1 > this.columns)
                {
                    errorCallback?.Invoke("row is wrong length in ccitt");
                    this.err = true;
                    a1 = this.columns;
                }
     ...
     * @private
     */
        private void _addPixelsNeg(int a1, int blackPixels)
        {
            ...
            else if (a1 < codingLine[codingPos])
            {
                if (a1 < 0)
                {
                    errorCallback?.Invoke("invalid code in ccitt a1: " + a1);
                    this.err = true;
                    a1 = 0;
                }
    ...
    */
     ...
     */
        private int _getTwoDimCode()
        {
            ...
            errorCallback?.Invoke("Bad two dim code in ccitt");
...
*/
        private int _getWhiteCode()
        {
            ...
            errorCallback?.Invoke("bad white code in ccitt");
            this._eatBits(1);
...
*/
        private int _getBlackCode()
        {
            ...
            errorCallback?.Invoke("bad black code in ccitt");
            this._eatBits(1);
